...
def kg_completion(rules, dataset, args):
    ...
    print("===============================")
    print("MRR: {:<8.4f} Hits@1: {:<8.4f} Hits@3: {:<8.4f} Hits@10: {:<8.4f}".format(np.mean(mrr), np.mean(hits_1), np.mean(hits_3),
                                                             np.mean(hits_10)))
...
def feq(relation, fact_rdf):
    ...
    for rdf in fact_rdf:
        ...
        if r == relation:
            count += 1
    return count
def evaluate_from_rules(args):
    dataset = Dataset(data_root=f'../datasets/{args.data}/', inv=True)

    all_rules, all_rule_heads = {}, []
    for L in range(args.rule_len_low, args.rule_len_high + 1):
        rule_dir = os.path.join("..", "rules", args.output_file)
        rule_path = os.path.join(rule_dir, f"{args.output_file}_{args.topk}_{L}.txt")
        load_rules(rule_path, all_rules, all_rule_heads)

    for head in all_rules:
        all_rules[head] = all_rules[head][:args.topk]
...
if __name__ == "__main__":
    ...
    for L in range(2,3):
        rule_dir = os.path.join("..", "rules", args.rule)
        file = os.path.join(rule_dir, f"{args.rule}_{args.topk}_{L}.txt")
